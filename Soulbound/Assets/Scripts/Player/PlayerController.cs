...
[Range(0,1)][SerializeField] float fCutJumpHeight;
    ...
    {  
        isTouchingWall = Physics2D.Raycast(wallCheck.position, transform.right * Player.Movement.GetDirection(), 0.25f, m_WhatIsGround);
        isTouchingLedge = Physics2D.Raycast(ledgeCheck.position, transform.right * Player.Movement.GetDirection(), 0.25f, m_WhatIsGround);
